<?php 
 ...
 if (isset($_GET['id'])){
	 $delete_testimony = DB::delete('testimony', "id_testimony=%i", $id = $_GET['id']);
	 if ($delete_testimony){
	   echo "<script>
			 alert('Testimony Berhasil Di Hapus');
 ...
 elseif (isset($_GET['Publish'])){
	$up = "published";
	$update_status = DB::update('testimony',[
		'status'=> $up,
	  ], "id_testimony=%i", $id = $_GET['Publish']);
	if ($update_status){
		echo "<script>
			  alert('Testimony Berhasil Di Published');
			  document.location='testimony.php';
			  </script>";
 }
}
 elseif (isset($_GET['Unpublish'])){
	 $upp = "pending";
	$update_statuss = DB::update('testimony',[
		'status'=> $upp,
	  ], "id_testimony=%i", $id = $_GET['Unpublish']);
	if ($update_statuss){
		echo "<script>
			  alert('Testimony Berhasil Di Pending');
			  document.location='testimony.php';
			  </script>";
 		}
}
...
<div id="db-wrapper">
				...
				<div class="col-lg-12 col-md-12 col-12">
					<div class="border-bottom pb-4 mb-4 d-lg-flex align-items-center justify-content-between">
						<div class="mb-2 mb-lg-0">
							<h1 class="mb-1 h2 font-weight-bold">All Quotes</h1>
						...
						<div>
							<a href="add-testimony.php" class="btn btn-primary">New Testimony</a>
						</div>
					</div>
				</div>
			</div>
								...
								<div class="tab-pane fade show active" id="all-post" role="tabpanel" aria-labelledby="all-post-tab">
									<div class="table-responsive border-0">
										...
										<tr>
											...
											<th scope="col" class="border-0">Quotes</th>
										...
										$awalData = ($jumlahDataPerHalaman * $halamanAktif) - $jumlahDataPerHalaman;
                                            $testimony = DB::query("SELECT * from testimony 
                                            INNER JOIN pengguna ON testimony.id_pengguna = pengguna.id_pengguna
                                            order by id_testimony desc LIMIT $awalData, $jumlahDataPerHalaman");
                                            $id = 0;
										...
										<tr>
											...
											<td class="align-middle border-top-0">
												<h5 class="mb-0"><a href="#!" class="text-inherit"><?php echo $testi['nama']; ?></a></h5>
											</td>
											<td class="align-middle border-top-0"><?php echo $testi['date_added'];?></td>
											<td class="align-middle border-top-0">						
												<?php echo $testi['Quote']; ?>																			
											</td>
											<td class="align-middle border-top-0">
												<?php $sts = $testi['status']; ?>
											...
											<td class="text-muted align-middle border-top-0 text-right">
												...
												<a class="dropdown-item" href="edit-testimony.php?id=<?=$testi['id_testimony']?>"><i class="fe fe-edit dropdown-item-icon"></i>Edit</a>
												<a class="dropdown-item" href="testimony.php?Publish=<?=$testi['id_testimony']?>"><i class="fe fe-toggle-left dropdown-item-icon"></i>Publish</a>
												<a class="dropdown-item" href="testimony.php?Unpublish=<?=$testi['id_testimony']?>"><i class="fe fe-toggle-right dropdown-item-icon"></i>Unpublish</a>
												<a class="dropdown-item" href="testimony.php?id=<?=$testi['id_testimony']?>" ><i class="fe fe-trash dropdown-item-icon"></i>Delete</a>
												</span>
												</span>
											</td>
										</tr>
	...
	<?php 
                                        ...
                                        $testimony = DB::query("SELECT * from testimony 
                                        INNER JOIN pengguna ON testimony.id_pengguna = pengguna.id_pengguna
                                        WHERE status = 'published' order by id_testimony desc LIMIT $awalData, $jumlahDataPerHalaman");
                                        $id = 0;
                                        foreach ($testimony as $testi):
                                        $id++;
										...
										<tr>
											...
											<td class="align-middle border-top-0">
												<a href="#!" class="text-inherit"><?php echo $testi['nama']; ?></a>
											</td>
											<td class="align-middle border-top-0"><?php echo $testi['date_added'];?></td>
											<td class="align-middle border-top-0">						
												<?php echo $testi['Quote']; ?>																			
								...
								<div class="tab-pane fade" id="draft" role="tabpanel" aria-labelledby="draft-tab">
									<div class="table-responsive border-0">
                                        ...
                                        $testimony = DB::query("SELECT * from testimony 
                                        INNER JOIN pengguna ON testimony.id_pengguna = pengguna.id_pengguna
                                        WHERE status = 'pending' order by id_testimony desc LIMIT $awalData, $jumlahDataPerHalaman");
                                        $id = 0;
                                        foreach ($testimony as $testi):
                                        $id++;
